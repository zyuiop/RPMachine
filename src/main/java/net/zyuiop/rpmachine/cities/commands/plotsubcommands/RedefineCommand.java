        ...
        if (PlayerSelection.getPlayerSelection(player) == null) {
            ...
            try {
                ...
                if (args.length < 1) {
                    ...
                    var oldIncluded = true;

                    for (Block b : plot.getArea()) {
                        if (!area.isInside(b.getLocation())) {
                            oldIncluded = false;
                            break;
                        }
                    }
                    ...
                    if (!oldIncluded) {
                        if (plot.getOwner() != null && !city.hasPermission(player, CityPermissions.REDEFINE_OCCUPIED_PLOT)) {
                            player.sendMessage(ChatColor.RED + "Vous ne pouvez pas rétrécir un plot occupé.");
                            return true;
                        } else if (plot.getOwner() == null && !city.hasPermission(player, CityPermissions.REDEFINE_EMPTY_PLOT)) {
                            player.sendMessage(ChatColor.RED + "Vous ne pouvez pas rétrécir un plot vide.");
                            return true;
                        }
                    }
