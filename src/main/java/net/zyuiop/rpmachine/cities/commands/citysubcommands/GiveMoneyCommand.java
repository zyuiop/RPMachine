				...
				if (args.length < 1) {
					...
					try {
						...
						if (!RPMachine.getInstance().getEconomyManager().canPay(player.getUniqueId(), amt)) {
							...
							RPMachine.getInstance().getEconomyManager().withdrawMoneyWithBalanceCheck(player.getUniqueId(), amt, new FinancialCallback() {
								@Override
								public void done(double newAmount, double difference) {
									if (difference < 0) {
